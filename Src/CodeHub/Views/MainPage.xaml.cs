...
namespace CodeHub.Views
{
    public sealed partial class MainPage : Windows.UI.Xaml.Controls.Page
    {
        ...
        private readonly SemaphoreSlim HeaderAnimationSemaphore = 
            new SemaphoreSlim(1);
        ...
        public MainPage(IActivatedEventArgs args)
        {
            ...
            Messenger.Default.Register<LocalNotificationMessageType>(
                this, RecieveLocalNotificationMessage
                );
            ...
            Messenger.Default.Register(this, delegate (AdsEnabledMessageType m) 
            { ViewModel.ToggleAdsVisiblity(); });
         
            Messenger.Default.Register(this, delegate (HostWindowBlurMessageType m) 
            {
                //ConfigureWindowBlur();
            });
           
            Messenger.Default.Register(this, delegate (UpdateUnreadNotificationMessageType m) 
            { 
                ViewModel.UpdateUnreadNotificationIndicator(m.IsUnread);
            });
           
            Messenger.Default.Register(this, async delegate (ShowWhatsNewPopupMessageType m) 
            {
            ...
            {
                return new NavigationService(AppFrame); 
            });
        ...
        private void MainPage_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //BlurBorderHamburger.Background 
        ...
        public void ConfigureWindowBlur()
        {
            if ( SettingsService.Get<bool>(SettingsKeys.IsAcrylicBlurEnabled) 
                ...
                && !ApiInformationHelper.IsMobileDevice )
            {
                //BlurBorder.Background = XAMLHelper.GetResourceValue<CustomAcrylicBrush>(
                //    "HostBackdropAcrylicBrush");
            }
            else
            {
        ...
        public async Task ConfigureHamburgerMenuBlur()
        {
            if (ApiInformationHelper.IsCreatorsUpdateOrLater)
            {
                if (!ApiInformationHelper.IsMobileDevice
                    && HamSplitView.DisplayMode == SplitViewDisplayMode.Inline)
                {
